...
export function Hero() {
  ...
  return (
    <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: 'var(--bg)' }}>
      ...
      <div className="page-container" style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', paddingTop: 'clamp(100px, 14vw, 140px)', paddingBottom: 'clamp(40px, 6vw, 80px)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 9999, border: '1px solid var(--border2)', background: 'var(--bg2)', marginBottom: 'clamp(24px, 4vw, 52px)' }}>
          <span style={{ position: 'relative', display: 'flex', width: 8, height: 8 }}>
            <span className="animate-ping" style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: '#6366f1', opacity: .5 }} />
            <span style={{ position: 'relative', width: 8, height: 8, borderRadius: '50%', background: '#6366f1' }} />
          </span>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5, delay: .55 }}
          ...
          <button onClick={() => document.getElementById('portfolio')?.scrollIntoView({ behavior: 'smooth' })}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: 'clamp(10px,1.5vw,13px) clamp(20px,3vw,30px)', borderRadius: 9999, background: '#4f46e5', border: 'none', fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 'clamp(13px,1.5vw,14px)', color: '#fff', transition: 'opacity .2s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.opacity = '.85' }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.opacity = '1' }}>
            View Case Studies <ArrowUpRight size={15} />
          </button>
          <a href="/resume.pdf" target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: 'clamp(10px,1.5vw,13px) clamp(20px,3vw,30px)', borderRadius: 9999, border: '1px solid var(--border2)', fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 'clamp(13px,1.5vw,14px)', color: 'var(--text2)', textDecoration: 'none', transition: 'border-color .2s, color .2s' }}
            onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'var(--text2)'; el.style.color = 'var(--text)' }}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5, delay: .7 }}
          ...
          <div className="stats-grid" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border2)', background: 'var(--bg1)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}>
            {STATS.map((s, i) => (
              <div key={s.l1}
                style={{ position: 'relative', padding: 'clamp(20px,3vw,32px) clamp(16px,2.5vw,28px)', borderRight: i < 2 ? '1px solid var(--border)' : 'none', background: 'var(--bg1)', transition: 'background 0.3s', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'var(--bg2)' }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'var(--bg1)' }}
              >
                <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(var(--border) 1px, transparent 1px), linear-gradient(90deg, var(--border) 1px, transparent 1px)', backgroundSize: '28px 28px', WebkitMaskImage: 'linear-gradient(to bottom left, rgba(0,0,0,1) 0%, rgba(0,0,0,0.5) 35%, transparent 70%)', maskImage: 'linear-gradient(to bottom left, rgba(0,0,0,1) 0%, rgba(0,0,0,0.5) 35%, transparent 70%)', pointerEvents: 'none' }} />
                <div style={{ position: 'relative', zIndex: 1, width: 40, height: 40, borderRadius: 10, background: 'var(--bg2)', border: '1px solid var(--border2)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24, flexShrink: 0, color: 'var(--text2)' }}>
                  <s.Icon size={18} strokeWidth={1.5} />
                </div>
                <div style={{ position: 'relative', zIndex: 1, fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", fontWeight: 700, fontSize: 'clamp(24px,3vw,40px)', color: 'var(--text)', lineHeight: 1, letterSpacing: '-0.03em', marginBottom: 12 }}>{s.num}</div>
                <p style={{ position: 'relative', zIndex: 1, fontFamily: "'Inter',sans-serif", fontWeight: 300, fontSize: 13, color: 'var(--text3)', lineHeight: 1.6, margin: 0 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
        ...
        <button
          ...
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.opacity = '1' }}>
          <span style={{ fontFamily: "'Geist Mono','Space Mono',monospace", fontSize: 9, letterSpacing: '.15em', color: 'var(--text2)', textTransform: 'uppercase' }}>Scroll</span>
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity }} style={{ color: 'var(--text2)' }}>
            <ArrowDown size={14} />
          </motion.div>
        </button>
      </motion.div>
